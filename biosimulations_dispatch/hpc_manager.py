...
class HPCManager:
    def __init__(
        ...
        self.ssh, self.ftp_client = self.__setup_ssh_ftp(
            host=server, username=username, password=password, sftp_host=sftp_server)
    ...
    def dispatch_job(
        ...
        """
        value_dict['tempDir'] = '{}/{}/{}'.format(
            temp_dir, value_dict['owner'], value_dict['id'])
        if simulator in self.allowed_biosimulators.keys():
            ...
            sbatch_remote = self.ftp_client.file(
                '{}/run.sbatch'.format(directory), 'w', -1)
            sbatch_remote.write(sbatch)
            sbatch_remote.flush()

            # Create SBML using given data and name on HPC inside subscriber's
            # simulation using simId
            sbml_remote = self.ftp_client.file(
                '{}/{}.xml'.format(directory, sbml_name), 'w', -1)
            sbml_remote.write(sbml)
            sbml_remote.flush()

            # Create SEDML using given data and name on HPC inside subscriber's
            # simulation using simId
            sedml_remote = self.ftp_client.file(
                '{}/{}.sedml'.format(directory, sedml_name), 'w', -1)
            sedml_remote.write(sedml)
            sedml_remote.flush()

            # Run the command to execute the simulation inside subscriber's
            # simulation dir using simId
            (stdin, stdout, stderr) = self.ssh.exec_command(
    ...
    def get_output_file(self, sim_spec: dict, local_path: str, temp_dir: str):
        ...
        # TODO: Check the Task DIR from task name when accepting more than 1
        # task for simulation
        ...
        complete_local_path = os.path.join(
            local_path, sim_spec['owner'], sim_spec['id'])
        for file in remote_files:
            if file.endswith('.ida') or file.endswith(
                    '.cvodeInput') or file.endswith('.xml'):
                self.ftp_client.get(
